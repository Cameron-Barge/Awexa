...
public class ChildProgressActivity extends AppCompatActivity {
        ...
        if (getIntent().getExtras() != null) {
            ...
            choreList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                    ...
                    intent.putExtra("choreId", chores.get(position).getChoreId());
            ...
            rewardsList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                    ...
                    intent.putExtra("rewardId", rewards.get(position).getRewardId());
                    intent.putExtra("childId", childId);
                    startActivity(intent);
                }
            });
    ...
    private void updateView(final Child c) {
        //update the title
        String title = getString(R.string.child_progress_title, c.getName());
        getSupportActionBar().setTitle(title);
        DatabaseReference choresDb = database.getReference("chores");
        dailyBar.setMax(c.getChores().keySet().size());
        ...
        for (final String choreId: c.getChores().keySet()) {
            Log.d("chore", choreId);
            choresDb.child(choreId)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot snapshot) {
                        ...
                        if ((chore.getDue() == null || chore.getDue().before(inAWeek)) && c.getChores().get(choreId)) {
                            numChoresForWeek[0]++;
                            weeklyBar.setMax(c.getChores().keySet().size());
                            weeklyBar.setProgress(numChoresForWeek[0]);
                        }
                        if ((chore.getDue() == null || chore.getDue().before(tomorrow)) && c.getChores().get(choreId)) {
                            numChoresForDay[0]++;
                            dailyBar.setMax(c.getChores().keySet().size());
        ...
        for (final String rewardId: c.getRewards().keySet()) {
